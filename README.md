...
| Usage data | Pikalytics AI endpoint, fetched live and cached in-process for an hour |
| Auth | Firebase Auth (Google and email/password), project `pokemon-vgc-tool` |
| Database | Firestore: `users/{uid}/teams` for the library, `aiQuota` for the free-tier counter |
...
credentials the free-tier counter lives in memory and resets on restart.
...
src/app/api/            Route handlers (all Node runtime; the calc dataset is 2.3 MB CJS)
  ...
  import, calc, learnset, usage, ruleset, ai-key
  showdown/{profile,replays,teams,team,share}
...
src/lib/ai/             llm (provider client), credential (BYOK + free tier), quota,
                        tools (shared tool declarations + legality gate), gemini (chat
                        runner), buildTeam, compareSet, benchmarks (role inference)
...
src/lib/showdown/       import, export, psApi
