...
function calculateChampions(gen, attacker, defender, move, field) {
    ...
    // Reg M-C additions: terrain Seeds consume themselves for +1 Def/SpD when their terrain is up.
    ...
    // Reg M-C addition: Air Balloon grants Ground immunity until popped (Gravity and Thousand Arrows bypass it).
...
function calculateBPModsChampions(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder, hit) {
    ...
    // Reg M-C addition: a type Gem boosts the first matching-type damaging move by 1.3x.
    if (basePower > 0 && !move.named('Struggle') && attacker.hasItem(move.type + ' Gem')) {
        bpMods.push(5325);
        desc.attackerItem = attacker.item;
    }
    return bpMods;
